...
import { Users, Mail, Award, Clock, CheckSquare } from "lucide-react";
import { getTeam } from "@/api";   // ✅ New API call
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTeamLead(data.users.find((u: any) => u.role === "lead"));
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold">
                    {teamMembers.length + (teamLead ? 1 : 0)}
                  </p>
              ...
              <div className="flex items-center space-x-4 p-4 border rounded-lg bg-gradient-to-r from-primary/5 to-transparent">
                <Avatar className="h-16 w-16">
                  <AvatarFallback className="bg-gradient-primary text-white text-lg font-semibold">
                    {teamLead.name?.split(" ").map((n: string) => n[0]).join("")}
                  </AvatarFallback>
                </Avatar>
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {teamLead.skills?.map((skill: string) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {teamMembers.length === 0 ? (
              <div className="col-span-2 text-center py-8">
                <p className="text-muted-foreground">
                  No team members found. Create user accounts with employee role to see team members.
                </p>
              </div>
            ) : (
                          ...
                          <div className="w-full bg-muted rounded-full h-2">
                            <div
                              className={`h-2 rounded-full transition-all duration-300 ${
                                (member.current_workload_hours || 0) /
                                  (member.weekly_capacity_hours || 40) >
                                0.8
                                  ? "bg-destructive"
                                  : (member.current_workload_hours || 0) /
                                      (member.weekly_capacity_hours || 40) >
                        ...
                        <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t">
                          <div className="text-center">
                            <p className="text-lg font-semibold">
                              {memberStats[member.id]?.activeTasks || 0}
                            </p>
                            <p className="text-xs text-muted-foreground">Active Tasks</p>
                          </div>
                          <div className="text-center">
                            <p className="text-lg font-semibold">
                              {memberStats[member.id]?.completedTasks || 0}
                            </p>
                            <p className="text-xs text-muted-foreground">Completed</p>
                          </div>
                          <div className="text-center">
                            <p className="text-lg font-semibold">
                              {memberStats[member.id]?.efficiency || 0}%
                            </p>
